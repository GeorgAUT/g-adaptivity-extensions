...
def RHS(m, X, Y, N, params):
    ...
    tau= 0.1# "speed" parameter in MMPDE5
...
def m(x, y, params):
    ...
    return (1 + diag_hessian(x, y, c_list, s_list))**params['mon_power']


def MA2d(x_comp, mesh, u, Hessian_Frob_u, opt, pde_params, SolverCoarse):
    ...
    def monitor(mesh):
        ...
        if opt['monitor_type'] == 'monitor_hessian_approx':
            ...
            PDESolver_coarse = SolverCoarse(opt, 2, mesh)
            PDESolver_coarse.update_solver(pde_params)

            u = PDESolver_coarse.solve()

            # This section for any info passed on GNN
            #save inputs to GNN
    ...
    if opt['mesh_geometry'] == 'headland1' or opt['mesh_geometry'] == 'headland2' or opt['mesh_geometry'] == 'headland05' or opt['mesh_geometry'] == 'H-shape':
        mover = MongeAmpereMover(mesh, monitor, method = "quasi_newton", rtol = 0.005, maxiter = 200)
    elif opt['mesh_geometry'] == 'L-shape':
        mover = MongeAmpereMover(mesh, monitor, method = "quasi_newton", rtol = 0.005, maxiter = 200)
    else:
        mover = MongeAmpereMover(mesh, monitor, method="relaxation", rtol=1e-4, maxiter=500)
...
def deform_mesh_ma2d(x_comp, coarse_mesh, PDESolver_coarse, u, Hessian_Frob_u, opt, pde_params, SolverCoarse):
    x_phys, j, build_time = MA2d(x_comp, coarse_mesh, u, Hessian_Frob_u, opt, pde_params, SolverCoarse)
    j=j+1 #to account for the initial mesh
    return x_phys, j, build_time
